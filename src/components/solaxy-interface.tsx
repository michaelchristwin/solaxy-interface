...
const SolaxyInterface: React.FC = () => {
  const [activeTab, setActiveTab] = useState<TransactionTab>("buy");
  const [inputMode, setInputMode] = useState<BuyMode>("stable");
  ...
  const getInputLabel = () => {
    if (activeTab === "buy") {
      ...
      // Sell mode
      if (inputMode === "slx") return "You pay";
      return "You receive";
    }
  };
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white dark:bg-gray-900 rounded-xl shadow-xl">
      ...
      {/* Action Button */}
      <button
        className="w-full mt-6 py-3.5 bg-yellow-500 hover:bg-yellow-600 dark:bg-yellow-600 dark:hover:bg-yellow-500 text-white font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        onClick={executeTransaction}
        disabled={!inputAmount || parseFloat(inputAmount) === 0}
      >
        {activeTab === "buy" ? "Buy SLX" : "Sell SLX"}
      </button>
